# execution-kubernetes-deploy Specification

## Purpose

Provide the Kustomize manifests and a dormant, manually-triggered GitHub
Actions deploy workflow needed to run `execution-service` on the shared EKS
cluster, plus a PR-time render check, without performing any real deploy as
a side effect of adding them.
