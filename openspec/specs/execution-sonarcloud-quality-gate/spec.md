...
Integrate a SonarCloud code quality scan, backed by persisted lcov coverage,
into the Execution Service's CI pipeline as a non-blocking step so quality
metrics are tracked without risking the required `ci` status check before
